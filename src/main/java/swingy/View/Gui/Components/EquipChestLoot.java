...
public class EquipChestLoot extends PanelComponent
{
    ...
    private TextArea    _items_text_area;
    private TextArea    _stats_text_area;

    public EquipChestLoot(int origin_x, int origin_y, int width, int height, Hero hero)
    {
        ...
        this._items_text_area = new TextArea(component_x, component_y, component_width, component_height, FONT_SIZE);
	...
	private void DisplayEquipmentItem(Item item, TextArea text_area)
	{
		...
		text_area.AddChunk(String.format("[%s] %s (", item.GetTypeStr(), item.GetName()), FG_COLOR);
    ...
    {
        this._items_text_area.Clear();

        this._items_text_area.AddChunk("Pick an item:\n\n", FG_COLOR);

        for (int i = 0; i < items.size(); i++)
        {
            this._items_text_area.AddChunk(String.format("%d: ", i + 1), FG_COLOR);
            DisplayEquipmentItem(items.get(i), this._items_text_area);
        }
    }
